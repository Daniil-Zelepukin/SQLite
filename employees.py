...
def add_employee():
    ...
    if name and position and salary:
        c.execute('INSERT INTO employees (name, position, department, email, phone, salary) VALUES (?, ?, ?, ?, ?, ?)',
...
def search_employee():
    ...
    if name:
        ...
        employee = c.fetchone()
        if employee:
            result_label.config(text=f"ID: {employee[0]},Имя: {employee[1]} , Должность: {employee[2]}, Email: {employee[4]}  ,Телефон: {employee[5]} , Зарплата: {employee[6]}")
        else:
...
show_employees_button = tk.Button(root, text="Показать всех сотрудников", command=show_all_employees)
